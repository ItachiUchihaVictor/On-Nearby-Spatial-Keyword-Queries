...
void Tree::getPointsRightHalf(node_d *v_node, point &p, point &p0, point &p1, int &flag)
{
...
void Tree::getPointsLeftHalf(node_d *v_node, point &q, point &q0, point &q1, int &flag)
{
	if ( v_node->left->type != LEAF_NODE )
	{
		flag = 0;
...
node_d *Tree::newSSLhLeft(node_d *current, point &q, point &q0, point &q1, bool &onepoint)
{
...
node_d *Tree::newSSUhRight(node_d *current, point &p, point &p0, point &p1, bool &onepoint)
{
	if ( current->type == LEAF_NODE )
	{
		if ( !onepoint )
...
node_d *Tree::newSSLhRight(node_d *current, point &q, point &q0, point &q1, bool &onepoint, bool &out)
{
	node_d *newcurrent = NULL;
	if ( current->type == LEAF_NODE )
	{
		out = true;
	}
...
node_d *Tree::newSSUhLeft(node_d *current, point &p, point &p0, point &p1, bool &onepoint, bool &out)
{
	node_d *newcurrent;
	newcurrent = NULL;
	if ( current->type == LEAF_NODE )
...
void Tree::updateBridgeHull(tree_node *v_node, const int &hull)
{
	...
	node_d *current_lh, *current_uh, *aux;
	bridgefound = false;
	onepoint_lh = true;
	onepoint_uh = true;
	if ( hull == 0 )
