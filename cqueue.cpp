...
void ConcatenableQueue::addQueuePointsToQueue(ConcatenableQueue *CQ, node_d *current_node)
{
...
node_d *ConcatenableQueue::rightmostNodeAtLevel(const int &level, node_d *bigger_y)
{
	node_d *currentNode;
	currentNode = root_;
...
node_d *ConcatenableQueue::leftmostNodeAtLevel(const int &level, node_d *smaller_y)
{
	node_d *currentNode;
	currentNode = root_;
...
void ConcatenableQueue::updateValuesDelete(node_d *current_node, const double &current_value, const double &new_value)
{
...
void ConcatenableQueue::updateValuesInsert(node_d *current_node, double value)
{
...
void ConcatenableQueue::deleteNode(node_d *node_to_delete)
{
	node_d *father, *brother, *fathersBrother;
...
{
	node_d *node_to_delete = searchLeafNode(value, root_);
...
node_d *ConcatenableQueue::searchLeafNode(const double &value, node_d *current)
{
...
void ConcatenableQueue::addChildNode(node_d *child, node_d *aux, const double &value)
{
	node_d *v1, *aux1;
	v1 = new node_d;
	...
	{
		node_d *newRoot = create2Node(root_->data2, v1->data2, child, v1);
		root_ = newRoot;
		child->parent = root_;
		v1->parent = root_;
	}
...
node_d *ConcatenableQueue::searchForInsert(node_d *new_node, node_d *current_node)
{
...
node_d *ConcatenableQueue::createLeafNode(const double &x_coord, const double &y_coord)
{
...
node_d *ConcatenableQueue::create2Node(const double &l_value, const double &m_value, node_d *left_child, node_d *right_child)
{
	node_d *newNode = new node_d;
...
void ConcatenableQueue::addNode(const double &x_coord, const double &y_coord) 
{
	...
	else
	{
		...
		{
			node_d *newNode1 = createLeafNode(x_coord, y_coord);
			node_d *newNode2 = createLeafNode(root_->x_coord,root_->data2);
		...
		else
		{
			...
			node_d *father = searchForInsert(newNode, root_);
...
void ConcatenableQueue::printValues(node_d *current_node)
{
	if ( current_node != NULL )
	{
		if ( current_node->type == LEAF_NODE)
...
{
	node_d *res = searchLeafNode(value, root_);
	if (res == NULL)
...
node_d *ConcatenableQueue::root()
{
	return root_;
}

void ConcatenableQueue::set_root(node_d *new_root)
{
	root_ = new_root;
}
...
ConcatenableQueue ConcatenableQueue::concatenate(ConcatenableQueue CQ1, ConcatenableQueue CQ2)
{
	...
	node_d *attachNode, *currentNode, *aux;
	if ( CQ1.isEmpty() )
		return CQ2;
	else if ( CQ2.isEmpty() )
		return CQ1;
