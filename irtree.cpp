...
void LinearPickSeeds( node_t* pnode, range* MBR, int& g1, int& g2)
{
	...
	{	
		...
		sep = max( highLowSide, lowHighSide) -
			  min( highLowSide, lowHighSide);

		//Recloc the normalized separation.
		width = max( MBR0[ i].max, MBR[ i].max) - 
				min( MBR0[ i].min, MBR[ i].min);
		sep /= width;
...
{
	//
	//IRTree_v.dim = cfg->dim;
	//M = cfg->M;
	//m = cfg->m;
	//IRTree_v.n_key = cfg->key_n;
	
	//strcpy( IRTree_v.tree_file, cfg->tree_file);
	//strcpy( IRTree_v.data_file, cfg->data_file);

	/*
	if( t_tag == 2)
	{
		IRTree_v.height_max = cfg->height_max;
		IRTree_v.is_first = ( bool*)malloc( sizeof( bool) * IRTree_v.height_max);
		for( i=0; i<IRTree_v.height_max; i++)
			IRTree_v.is_first[i] = true;
	}
	*/
